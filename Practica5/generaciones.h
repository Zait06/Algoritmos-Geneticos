...
void rankeoBool(void)
{
	float aux1=1.1,aux2=2-1.1; 
	int paro,paro1;
	int *orpos;

	orpos=(int *)malloc(inds*sizeof(int *));
	...
	{
		paro=orpos[i];
		aprob[paro]=aux2+((aux1-aux2)*(i/inds-1));
	}
...
void ruletaBool(void)
{
	...
	/*printf("\nSeleccion de los padres para la cruza por el metodo de la ruleta.\n");
	printf("Se crearan %d valores aleatorios: r[0.00 - %.02f]\n",inds,sumProb);*/
	...
	for(i=0;i<inds;i++)
	{
		...
		for(j=0;j<inds;j++)
		{
			...
			if(suau>r)
			{
				...
				//printf("Para r[%d]=%.02f, el seleccionado es el Individuo %d\n",i+1,r,j+1);
				break;
			}
		}
		suau=0;
